...
class Plugin_Notice {
	...
	private function load_hooks() {
		...
		add_action( 'wp_ajax_sociatic_dismiss_notice', array( $this, 'dismiss_notice' ) );
	}
	...
	public function notice_install_plugin() {
		...
		if ( get_option( 'sociatic_dismiss_notice' ) ) {
			return;
		}
  ...
  '1x' => 'https://ps.w.org/gutenverse-companion/assets/icon-128x128.png?rev=3162415',
		...
		wp_register_style( 'sociatic-theme-notice', false );
		wp_enqueue_style( 'sociatic-theme-notice' );
		ob_start();
		?>
		
				.sociatic-simple-notice {
				
					padding: 24px !important; 
				
					border-left: 4px solid #007cba !important;
				}
				.sociatic-simple-notice p:first-child {
				
					margin-bottom: 20px; 
					margin-top: 0;
				}
				.sociatic-simple-notice p:last-of-type {
				
					margin-top: 0; 
					margin-bottom: 0;
				}
				.sociatic-simple-notice .sociatic-notice-title {
					font-size: 17px;
					font-weight: 600; 
					display: block;
					margin-bottom: 6px;
				}
				.sociatic-simple-notice .sociatic-notice-description {
					font-size: 13px;
					display: block;
				...
				.sociatic-simple-notice .button-primary {
					padding: 4px 16px !important;
				}

			
				.sociatic-simple-notice .notice-dismiss {
					background: none !important;
					box-shadow: none !important;
					opacity: 1;
				}
				
				.sociatic-simple-notice .notice-dismiss:before {
					color: #a7aaad;
					padding: 0; 
				}
				
				.sociatic-simple-notice .notice-dismiss:hover:before,
				.sociatic-simple-notice .notice-dismiss:focus:before {
					color: #c90000;
				}
		...
		wp_add_inline_style( 'sociatic-theme-notice', $custom_style );
		wp_register_script(
			'sociatic-theme-notice',
			'',
			array( 'wp-api-fetch' ),
			'1.0.0',
			true
		...
		wp_enqueue_script( 'sociatic-theme-notice' );
		...
		function sequenceInstall(plugin, pluginsInstalled) {
			return new Promise((resolve, reject) => {
				...
				const needUpdate = plugin.installed
					? SOCIATICPluginUtils.isVersionGreater(plugin.req_version, pluginsInstalled[`${path}.php`].Version)
					: false;
		...
		document.addEventListener('DOMContentLoaded', () => {
			const notice = document.querySelector('.notice.is-dismissible.sociatic-simple-notice');
				...
				setTimeout(() => {
					...
					if (dismissBtn) {
						dismissBtn.addEventListener('click', (e) => {
							e.preventDefault();
							jQuery.post(ajaxurl, {
								action: 'sociatic_dismiss_notice',
								_ajax_nonce: nonce
							});
						});
					}
			...
			button.addEventListener('click', function (e) {
				...
				button.innerHTML = `<?php esc_html_e( 'Installing...', 'sociatic' ); ?>`;
				...
				if (!hasFinishClass) {
					...
					sequence.then(() => {
						...
						button.innerHTML = `<?php esc_html_e( 'Install Failed, Try Again', 'sociatic' ); ?>`;
		...
		wp_add_inline_script( 'sociatic-theme-notice', $custom_script );

		?>
		<div class="notice notice-info is-dismissible sociatic-simple-notice" data-nonce="<?php echo esc_attr( wp_create_nonce( "sociatic_dismiss" ) ); ?>">
				<p>
					<strong class="sociatic-notice-title"><?php esc_html_e( "Thank you For Installing Sociatic Theme", "sociatic" ); ?></strong>
					<span class="sociatic-notice-description">
						<?php esc_html_e( "Unlock the full potential of your website with the recommended plugins.", "sociatic" ); ?>
						<br/>
						<?php esc_html_e( "Activate it to explore exclusive extensions, ready-to-use demo templates, and powerful features that make building your site easier and more enjoyable.", "sociatic" ); ?>
					</span>
				</p>
				<p>
					<a href="#" class="button button-primary" id="gutenverse-install-plugin"><?php echo esc_html__( "Install Recommended Plugins", "sociatic" ); ?></a>
				...
				<p class="installing-warning" style="font-size: 12px; color: #666; display: none; margin-top: 5px;">
					<i><?php esc_html_e( "Don\'t refresh the page when installing recommended plugins", "sociatic" ); ?></i>
				</p>
			</div>
		<?php
	}
	...
	public function dismiss_notice() {
		check_ajax_referer( 'sociatic_dismiss' );

		if ( ! get_option( 'sociatic_dismiss_notice' ) ) {
			update_option( 'sociatic_dismiss_notice', true );
		}
